...
const WorldMap: FC = () => {
  ...
  return (
    <article className="card card-container h-full">
      ...
      <div className="card-body h-full">
        <ComposableMap projectionConfig={{ center: [29.8738, 26.8206], scale: 3500 }}>
          ...
          {mapMarkers.map(({ coordinates, name }: any) => (
            <Marker key={name} coordinates={coordinates}>
              <svg viewBox="0 0 24 24" width="35" height="35" fill={themeColors.danger}>
              ...
              <text y="50" x="14" fontSize={22} textAnchor="middle" fill={"#fff"} fontWeight="bold">
                {name}
              </text>
            </Marker>
          ))}
